...
public class ConfigurationFile {
    ...
    private String uniqueId, phonenumber, environment;
                                                        ...
                                                        if (reallyKill) {
                                                            ...
                                                            try {
                                                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                                                    context.stopService(new Intent(context, io.okverify.android.services.LocationService.class));
                                                                } else {
                                                                    context.stopService(new Intent(context, io.okverify.android.services.LocationService.class));
                                                                }
                                                                stopPeriodicPing();
                                                                ...
                                                                if (noForeground.length() > 0) {
                                                                    ...
                                                                    if (noForeground.toLowerCase().contains(brand.toLowerCase())) {
                                                                        ...
                                                                        try {
                                                                            /*
                                                                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                                                                context.startForegroundService(new Intent(context, io.okverify.android.services.LocationService.class));
                                                                            } else {
        ...
        try {
            ...
            if (tempPhonenumber != null) {
                if (tempVerify) {
                    ...
                    io.okverify.android.callback.GetAddressCallBack getAddressCallBack = new io.okverify.android.callback.GetAddressCallBack() {
                        @Override
                        public void querycomplete(String response, boolean me) {
                            if (me) {
                                displayLog("good response " + response);
                    ...
                    io.okverify.android.asynctask.GetAddressTask getAddressTask = new io.okverify.android.asynctask.GetAddressTask(context, getAddressCallBack);
                    getAddressTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                }
            }
        } catch (Exception e) {
